ig.module('game.entities.dragon').requires('game.entities.base', 'game.entities.attack').defines(function() {
      ...
      if (this.playerNumber === 1) {
        ...
        if (ig.game.controller1Present) {
          ...
          if (ig.game.pressedSquare === true) {
            ...
            for (var i = 0; i < 4; i++)
            ig.game.spawnEntity(EntityAttack, this.pos.x + Math.random() * 10 + (this.flip? 10: 7), this.pos.y + Math.random() * 10 - 10, {
              'flip': this.flip,
              'type': 0
            });
            if (this.flip) {
          ...
          if (ig.input.state('attack')) {
            ...
            ig.game.shakeAmplitude = Math.min(Math.max(ig.game.shakeAmplitude += 5, 0),7);
            for (var i = 0; i < 4; i++)
            ig.game.spawnEntity(EntityAttack, this.pos.x + Math.random() * 10 + (this.flip? 10: 7), this.pos.y - 10, {
              'flip': this.flip,
              'type': 0
            });
        ...
        if (ig.game.controller2Present) {
          ...
          if (ig.game.pressedSquare_2 === true) {
            ...
            ig.game.shakeAmplitude = Math.min(Math.max(ig.game.shakeAmplitude += 5, 0),7);
            for (var i = 0; i < 4; i++)
            ig.game.spawnEntity(EntityAttack, this.pos.x + Math.random() * 10 + (this.flip? 10: 7), this.pos.y, {
              'flip': this.flip,
              'type': 0
            });
            if (this.flip) {
